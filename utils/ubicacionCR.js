// División territorial administrativa de Costa Rica: 7 provincias, 84
// cantones, sus distritos. Se usa para los selectores en cascada del
// formulario (provincia -> cantón -> distrito) y para los filtros de la
// lista de propiedades. Misma lista que la app móvil (utils/ubicacionCR.js).
...
export const PROVINCIAS = Object.keys(PROVINCIAS_CR);

export function cantonesDe(provincia) {
  return provincia && PROVINCIAS_CR[provincia] ? Object.keys(PROVINCIAS_CR[provincia]) : [];
}

export function distritosDe(provincia, canton) {
  return provincia && canton && PROVINCIAS_CR[provincia]?.[canton] ? PROVINCIAS_CR[provincia][canton] : [];
}
